...
function EnrollScanContent() {
  ...
  const captureAndSend = useCallback(async () => {
    if (!videoRef.current || !canvasRef.current || sending) return;
    setSending(true);
        ...
        <div className="mt-6 text-center">
          {isComplete ? (
            <div className="flex flex-col items-center gap-4">
              <Link
                href="/"
                className="group w-full inline-block text-center bg-emerald-500 text-white px-8 py-3.5 rounded-full text-sm font-semibold hover:bg-emerald-600 transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              >
                Done
